...
function ensureBrandHeader(html){if(/<header\b/i.test(html)) return html;return html.replace(/<body([^>]*)>/i,`<body$1>${BRAND_HEADER}`);}

let records=wards.map(w=>{const p=provinceByCode.get(String(w.parent_code));if(!p) throw new Error(`Không tìm thấy tỉnh cho mã xã ${w.code}`);const type=typeLabel(w);const name=String(w.name||'').trim();if(!name) throw new Error(`Đơn vị ${w.code} thiếu tên`);const baseSlug=w.slug||strip(name);return {code:String(w.code),province:displayProvince(p),provinceSlug:p.slug||strip(p.name),provinceCode:String(p.code),type,name,slug:baseSlug,baseSlug,path:w.path_with_type||w.path||`${type} ${name}, ${displayProvince(p)}`};});
const baseCounts=new Map();
for(const r of records){const k=`${r.provinceSlug}/${r.type}/${r.baseSlug}`;baseCounts.set(k,(baseCounts.get(k)||0)+1);}
records=records.map(r=>{const k=`${r.provinceSlug}/${r.type}/${r.baseSlug}`;const duplicate=baseCounts.get(k)>1;return {...r,duplicate,slug:duplicate?`${r.baseSlug}-${r.code}`:r.baseSlug};});
...
function localityPage(r){
  ...
  const title=`Tuyển thợ mỏ cho lao động ${place}${r.duplicate?` – mã địa bàn ${r.code}`:''}`;
  const url=`https://thaylinhtuyenthomo.vn/viec-lam-nganh-than/${r.provinceSlug}/${r.type}/${r.slug}/`;
  const apply=`/viec-lam/cong-nhan-mo-ham-lo-quang-ninh/?province=${encodeURIComponent(r.province)}&locality=${encodeURIComponent(r.name)}&locality_code=${encodeURIComponent(r.code)}&utm_source=google&utm_medium=organic&utm_campaign=commune_jobs#dang-ky`;
...
for(const [pSlug,list] of byProvince){
  list.sort((a,b)=>a.name.localeCompare(b.name,'vi')||a.code.localeCompare(b.code));
  const p=list[0].province;
  const dir=path.join(out,pSlug,'xa-phuong');fs.mkdirSync(dir,{recursive:true});
  const items=list.map(r=>`<li><a href="../${r.type}/${r.slug}/">${esc(r.type)} ${esc(r.name)}${r.duplicate?` <small>(mã ${esc(r.code)})</small>`:''}</a></li>`).join('');
  const schema={'@context':'https://schema.org','@type':'ItemList',numberOfItems:list.length,itemListElement:list.map((r,i)=>({'@type':'ListItem',position:i+1,name:`${r.type} ${r.name}${r.duplicate?` – mã ${r.code}`:''}`,url:`https://thaylinhtuyenthomo.vn/viec-lam-nganh-than/${pSlug}/${r.type}/${r.slug}/`}))};
  fs.writeFileSync(path.join(dir,'index.html'),`<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Tuyển thợ mỏ theo xã, phường ${esc(p)} | Thầy Linh</title><meta name="description" content="Tra cứu tuyển thợ mỏ theo toàn bộ ${list.length} xã, phường, đặc khu tại ${esc(p)}; đăng ký từ địa phương, học nghề và làm việc thực tế tại Quảng Ninh."><meta name="robots" content="index,follow"><link rel="canonical" href="https://thaylinhtuyenthomo.vn/viec-lam-nganh-than/${pSlug}/xa-phuong/"><link rel="stylesheet" href="/styles.css"><script type="application/ld+json">${JSON.stringify(schema)}</script></head><body>${BRAND_HEADER}<main class="section"><p><a href="/viec-lam-nganh-than/${pSlug}/">← Tuyển dụng ${esc(p)}</a></p><h1>Tuyển thợ mỏ theo xã, phường tại ${esc(p)}</h1><p>Đã phủ đủ ${list.length} đơn vị cấp xã hiện hành của ${esc(p)}. Chọn đúng địa bàn đang sinh sống để giữ nguồn tuyển; nơi học và làm việc thực tế là Quảng Ninh.</p><ul>${items}</ul></main></body></html>`);
  const provincePage=path.join(out,pSlug,'index.html');
  if(fs.existsSync(provincePage)){let source=fs.readFileSync(provincePage,'utf8');source=ensureBrandHeader(source);if(!source.includes(`/${pSlug}/xa-phuong/`)){source=source.replace('</main>',`<section class="section"><h2>Tuyển thợ mỏ theo từng xã, phường</h2><p><a href="/viec-lam-nganh-than/${pSlug}/xa-phuong/">Xem toàn bộ ${list.length} xã, phường, đặc khu của ${esc(p)} →</a></p></section></main>`);}fs.writeFileSync(provincePage,source);}
}
...
fs.writeFileSync(path.join(ROOT,'local-coverage.json'),JSON.stringify({official_source:OFFICIAL_SOURCE,data_source:{province:PROVINCE_DATA,locality:WARD_DATA},generated_at:new Date().toISOString(),communes:records.length,duplicate_name_groups:[...baseCounts.values()].filter(n=>n>1).length,legacy_district_names:0,by_province:counts},null,2));
console.log(JSON.stringify({status:'ok',communes:records.length,provinces:byProvince.size,duplicate_name_groups:[...baseCounts.values()].filter(n=>n>1).length,by_province:counts},null,2));

(() => {
  ...
  function normalize(value) {
    return String(value || "").toLocaleLowerCase("vi").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/đ/g, "d").replace(/[^a-z0-9]+/g, " ").trim();
  }
  ...
  function targetUrl() {
    ...
    if (!out.searchParams.has("utm_content")) out.searchParams.set("utm_content", `paid_bridge_${detectIntent()}`);
    out.hash = "dang-ky";
    return out.toString();
  }
  ...
  [["[data-ads-intent-kicker]", copy[0]], ["[data-ads-intent-title]", copy[1]], ["[data-ads-intent-lead]", copy[2]]].forEach(([selector, text]) => {
  ...
  document.querySelectorAll("[data-ads-continue]").forEach((link) => {
    ...
    link.addEventListener("click", () => {
      ...
      window.dataLayer.push({ event: "google_ads_bridge_click", paid_search_intent: intent, page_path: location.pathname, destination_path: TARGET_PATH });
    });
  });

  window.dataLayer = window.dataLayer || [];
  window.dataLayer.push({ event: "google_ads_bridge_view", paid_search_intent: intent, page_path: location.pathname, has_google_click_id: Boolean(params.get("gclid") || params.get("gbraid") || params.get("wbraid")) });
})();

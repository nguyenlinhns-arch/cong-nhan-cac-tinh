(() => {
  ...
  const state = { data: null, details: null, active: 'overview' };
  ...
  function render() {
    ...
    if (!state.data || !state.details) return;
    const map = { overview: renderOverview, quality: renderQuality, ph: renderPH, dn: renderDN, dv: renderDV };
    root.innerHTML = (map[state.active] || renderOverview)();
  }

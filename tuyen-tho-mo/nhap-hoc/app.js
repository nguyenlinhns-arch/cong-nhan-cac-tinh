(() => {
  ...
  function renderSummary() {
    ...
    if (source) { source.textContent = s.source_system_status || 'CHƯA XÁC ĐỊNH NGUỒN'; source.className = `source-health ${ok ? 'ok' : 'warn'}`; }
    if (notice) notice.className = `status-line ${ok ? 'live' : 'warning'}`;
    if (updated) updated.textContent = `Dữ liệu tổng hợp: ${state.data?.generated_at || s.data_version || ''}`;
  }
  ...
  function markMonthHeader() {
    ...
    document.querySelectorAll('.month-head').forEach(el => el.classList.toggle('current-month', Number(el.dataset.month) === m));
  }
  ...
  function renderTable(kind) {
    ...
    if (!rows.length) { body.innerHTML = '<tr><td colspan="17" class="loading-cell">Chưa có dữ liệu báo cáo.</td></tr>'; return; }
    ...
    rows.forEach(row => {
      ...
      const tt = document.createElement('td'); tt.textContent = row.total_row ? 'TỔNG CỘNG' : row.tt; tr.appendChild(tt);
      const entity = document.createElement('td'); entity.className = 'entity'; entity.textContent = row.total_row ? 'TỔNG CỘNG' : (row.name || row.code || '');
      if (!row.total_row && row.code) { const small = document.createElement('small'); small.textContent = row.code; entity.appendChild(small); }
      tr.appendChild(entity);
      (row.months || []).forEach((v, i) => { const td = document.createElement('td'); td.textContent = fmt(v); if (i + 1 === mNow) td.classList.add('current-month'); tr.appendChild(td); });
      [row.total, row.active, row.inactive].forEach(v => { const td = document.createElement('td'); td.textContent = fmt(v); tr.appendChild(td); });
      body.appendChild(tr);
    });
  ...
  function renderAll() { renderSummary(); markMonthHeader(); renderTable(state.active); }

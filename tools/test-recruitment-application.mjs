...
async function runCase({ height, health, expected }) {
  const listeners = {};
  const tracked = [];
  const delivered = [];
  ...
  context.window = {
    ...
    setTimeout,
    clearTimeout,
    THAY_LINH_RECRUITMENT: { endpoint: "https://example.test/exec", timeoutMs: 12000 },
  };
  ...
  for (const pii of ["Nguyễn Văn Kiểm Thử", "0963048585", "2000-01-01", "Hồ Chí Minh", "165", "58"]) {
...
const results = [];
results.push(await runCase({ height: 165, health: "Sức khỏe tốt, sẵn sàng khám tuyển", expected: "eligible" }));
results.push(await runCase({ height: 165, health: "Cần trao đổi thêm trước khi khám", expected: "needs_review" }));
results.push(await runCase({ height: 150, health: "Sức khỏe tốt, sẵn sàng khám tuyển", expected: "not_eligible" }));

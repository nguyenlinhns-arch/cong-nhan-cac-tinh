...
if (!addressMatches) throw new Error("Could not locate the admission address card");
html = html.replace(
  addressPattern,
...
const registerMatches = html.match(registerPattern);
if (!registerMatches || (html.match(new RegExp(registerPattern.source, "g")) || []).length !== 1) {
  throw new Error("Could not locate one registration action row");
}
html = html.replace(
  registerPattern,

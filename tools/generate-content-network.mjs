...
const recruitmentReview = JSON.parse(fs.readFileSync(path.resolve("content/recruitment-review-v10.json"), "utf8"));
const activeOccupationProfiles = recruitment.occupation_profiles.filter((profile) => profile.active_intake);
...
const reviewedDate = recruitmentReview.reviewed_at;
const factsModifiedDate = recruitmentReview.verification_content_modified || reviewedDate;
...
const branchCards = [
  ...
  ["Việc làm đang tuyển", "Ba nghề mỏ hầm lò, điều kiện và quyền lợi được trình bày theo một nguồn dữ liệu thống nhất.", "/viec-lam-nganh-than/"],
...
const centralBody = `<div class="network-wrap network-facts"><div><strong>${allArticles.length}</strong><span>bài có nguồn và chủ đề rõ</span></div><div><strong>${provinces.length}</strong><span>điểm vào theo tỉnh</span></div><div><strong>${activeOccupationProfiles.length} nghề</strong><span>đang tiếp nhận năm 2026</span></div><div><strong>1 biểu mẫu</strong><span>giữ tỉnh và nguồn liên hệ</span></div></div>
  ...
  "@graph": [
    {
      ...
      dateModified: factsModifiedDate,
...
const currentFactsQuickDecisions = [
  ...
  },
  {
    ...
    text: `Khai thác, xây dựng mỏ học 2–3 tháng; cơ điện mỏ học 10 tháng tại ${recruitment.contact.admission_address}; chỉ di chuyển khi đã có lịch tiếp nhận.`,
...
writePage("thong-tin-tuyen-tho-mo", page({pathName: currentFactsPath, title: currentFactsTitle, description: currentFactsDescription, eyebrow: "THÔNG TIN CHÍNH THỨC ĐANG ÁP DỤNG", heading: "Tuyển thợ mỏ tháng 8/2026: trả lời 15 câu hỏi", lead: "Nam 18–40 tuổi, cao từ 1m53, nặng từ 47 kg; khai thác và xây dựng mỏ học 2–3 tháng, cơ điện mỏ học 10 tháng; Thu nhập 20–25 triệu đồng/tháng khi hoàn thành định mức lao động.", body: currentFactsBody, schema: currentFactsSchema}));

const jobsDescription = "Việc làm công nhân mỏ, thợ lò và cơ điện mỏ tại Quảng Ninh cho nam 18–40 tuổi; xem ba nghề đang tuyển và thông tin tư vấn dành cho người lao động tại 26 tỉnh.";
const jobItems = [
  ...activeOccupationProfiles.map((profile) => ({title: profile.title, urlPath: `viec-lam/${profile.slug}`})),
  ...provinces.map((province) => ({title: `Việc làm ngành Than cho người ${province.name}`, urlPath: `viec-lam-nganh-than/${province.slug}`})),
];
const occupationCards = activeOccupationProfiles.map((profile, index) => `<article class="network-card${index === 0 ? " network-card--accent" : ""}"><div class="network-card__body"><small>VỊ TRÍ ${String(index + 1).padStart(2, "0")} · HỌC ${esc(profile.training_duration_current)}</small><h2>${esc(profile.title)}</h2><p>${esc(profile.summary)}</p><a href="/viec-lam/${profile.slug}/">Xem tin việc làm →</a></div></article>`).join("");
const jobsBody = `<div class="network-wrap network-facts"><div><strong>18–40</strong><span>độ tuổi nam đang tiếp nhận</span></div><div><strong>1m53</strong><span>chiều cao tối thiểu</span></div><div><strong>47 kg</strong><span>cân nặng tối thiểu</span></div><div><strong>2–3 / 10 tháng</strong><span>thời gian học theo nghề</span></div></div>
<section class="network-section"><div class="network-wrap"><div class="network-heading"><div><p class="network-eyebrow">VỊ TRÍ ĐANG TIẾP NHẬN</p><h2>Ba hướng nghề mỏ hầm lò tại Quảng Ninh</h2></div><p>Không yêu cầu kinh nghiệm. Người học được đào tạo trước khi doanh nghiệp tiếp nhận nếu tốt nghiệp đạt yêu cầu.</p></div><div class="network-grid">${occupationCards}<article class="network-card"><div class="network-card__body"><small>CHÍNH SÁCH</small><h2>Học nghề trước, nhận việc sau</h2><p>Miễn kinh phí đào tạo theo chỉ tiêu; bố trí ăn ở, hỗ trợ 7,5 triệu đồng/tháng và Thu nhập 20–25 triệu đồng/tháng khi hoàn thành định mức lao động.</p><a href="/cam-nang-nghe-mo/">Đọc cẩm nang →</a></div></article></div></div></section>
...
writePage("viec-lam-nganh-than", page({pathName: "/viec-lam-nganh-than/", title: "Việc làm công nhân mỏ, thợ lò theo tỉnh", description: jobsDescription, eyebrow: "VIỆC LÀM NGÀNH THAN 2026", heading: "Việc làm công nhân mỏ, thợ lò theo tỉnh", lead: "Ba nghề khai thác mỏ, xây dựng mỏ và cơ điện mỏ đang tiếp nhận; người chưa có kinh nghiệm được đào tạo từ đầu tại Quảng Ninh trước khi nhận việc.", body: jobsBody, schema: commonSchema({pathName: "/viec-lam-nganh-than/", title: "Việc làm công nhân mỏ, thợ lò theo tỉnh", description: jobsDescription, items: jobItems})}));

const guideDescription = "Cẩm nang học nghề mỏ: điều kiện 18–40 tuổi, sức khỏe, hồ sơ, khai thác và xây dựng mỏ học 2–3 tháng, cơ điện mỏ học 10 tháng, thu nhập và đời sống tại Quảng Ninh.";
...
<section class="network-section network-section--soft"><div class="network-wrap"><div class="network-heading"><div><p class="network-eyebrow">CAM KẾT BIÊN TẬP</p><h2>Thông tin phải giúp người lao động ra quyết định tốt hơn</h2></div><p>Mọi nội dung đều quay về ba câu hỏi: điều kiện có phù hợp, quyền lợi nào đang áp dụng và bước tiếp theo an toàn là gì.</p></div><ul class="network-list"><li><b>1</b><div><strong>Thống nhất dữ liệu tuyển sinh</strong><span>Tuổi, thể lực, sức khỏe, thời gian học và quyền lợi lấy từ cùng một hồ sơ chính sách.</span></div></li><li><b>2</b><div><strong>Viết lại và ghi nguồn</strong><span>Bài báo được biên soạn lại với lượng thông tin gần bản nguồn, giữ đúng dữ kiện và đặt liên kết bài gốc ở cuối để người đọc đối chiếu.</span></div></li><li><b>3</b><div><strong>Bảo vệ dữ liệu ứng viên</strong><span>Đo nguồn liên hệ nhưng không đưa thông tin nhận dạng cá nhân vào GA4, Meta hoặc liên kết chia sẻ.</span></div></li></ul><div class="network-actions"><a class="network-button network-button--outline" href="${editorialPolicyPath}">Đọc đầy đủ nguyên tắc biên tập</a></div></div></section>

...
function splitLongSentence(sentence) {
  ...
  if (words.length <= 34) return [sentence];
  const clauses = sentence.split(/;\s+|,\s+(?=(?:trong khi|còn|đồng thời|tuy nhiên|vì vậy|qua đó|do đó|nhưng)\b)/iu).map((part) => part.trim()).filter(Boolean);
  if (clauses.length < 2) return [sentence];
  return clauses.map((part) => /[.!?]$/u.test(part) ? part : `${part}.`);
}

function cleanParagraph(value = "") {
  const sentences = stripTags(value)
    .split(/(?<=[.!?])\s+/u)
    .map(cleanSentence)
    .filter(Boolean)
...
function mergeFragments(paragraphs) {
  ...
  for (const paragraph of paragraphs) {
    if (wordCount(paragraph) < 18 && output.length) {
      output[output.length - 1] = `${output[output.length - 1]} ${paragraph}`.replace(/\s+/g, " ");
    } else {
      output.push(paragraph);
    }
  }
  if (output.length > 1 && wordCount(output.at(-1)) < 18) {
    output[output.length - 2] = `${output[output.length - 2]} ${output.pop()}`.replace(/\s+/g, " ");
  }
  return output;
}
...
function removeFrom(value, fragments) {
...
function groupParagraphs(article) {
  ...
  const lede = cleanParagraph(override.lede || intro[0] || article.lead || article.description || "");
  const nutgraph = cleanParagraph(override.nutgraph || intro[1] || article.description || article.lead || "");
  const used = [lede, nutgraph].filter(Boolean);
  const sectionParagraphs = sections.flatMap((section) => section.paragraphs)
    .filter((paragraph) => !used.some((item) => similarity(item, paragraph) >= 0.82));
  const remainingIntro = intro.slice(2).filter((paragraph) => !used.some((item) => similarity(item, paragraph) >= 0.82));
  const body = mergeFragments(dedupe([...remainingIntro, ...sectionParagraphs]));
  const midpoint = Math.max(2, Math.ceil(body.length / 2));
  const first = body.slice(0, midpoint);
  const second = body.slice(midpoint);
  const heading1 = cleanHeading(override.heading1 || sections[0]?.title || article.factsTitle || "Diễn biến và dữ kiện chính");
  const heading2 = cleanHeading(override.heading2 || sections.at(-1)?.title || article.conclusionTitle || "Ý nghĩa đối với người lao động");
  const ending = cleanParagraph(article.takeaway || article.conclusion || "");
  return {lede, nutgraph, first, second, heading1, heading2, ending};
}
...
function renderBody(article, existingBody) {
  ...
  const story = groupParagraphs(article);
  if (!story.lede || !story.nutgraph) return "";
  ...
  const bodyWords = wordCount(copyParts.join(" "));
  const paragraphCount = (copyParts.join(" ").match(/<p(?:\s|>)/gi) || []).length;
  if (bodyWords < 300 || paragraphCount < 6 || paragraphCount > 18) return "";
...
function polishHero(html, article) {
  ...
  if (!lead) return html;
  return html.replace(/<p class="lead">[\s\S]*?<\/p>/i, `<p class="lead">${escapeHtml(lead)}</p>`);
}
...
for (const article of registry.values()) {
  ...
  const rewrittenBody = renderBody(article, existingBody);
  if (!rewrittenBody) continue;
  let after = before.replace(articleBodyPattern, rewrittenBody);

...
const occupationSource = master.source_documents.find((source) => source.id === "554-HD-CDTKV");
const roles = master.occupation_profiles
  .filter((profile) => profile.active_intake)
  .map((profile) => ({ ...profile, responsibility: profile.summary }));
const responsibilityText = (role) => role.responsibilities.map((item) => item.replace(/[.;]+$/u, "")).join("; ");
...
function plainDescription(role) {
  return `${master.recruitment_coordinator} phối hợp tuyển lao động nam học nghề ${role.short} để làm việc tại các doanh nghiệp thuộc TKV ở Quảng Ninh. ${role.responsibility} Thời gian học ${role.training_duration_current}. Điều kiện: nam ${ageLabel} tuổi, cao từ ${heightLabel}, nặng từ ${criteria.weight_min_kg} kg, sức khỏe tốt, không cận thị và không mắc bệnh tim mạch, huyết áp hoặc bệnh về mắt ảnh hưởng đến công việc; không yêu cầu kinh nghiệm. Người học thuộc chỉ tiêu được miễn kinh phí đào tạo, bố trí ba bữa/ngày với mức ăn 90.000 đồng/ngày, ký túc xá và hỗ trợ 7,5 triệu đồng/tháng theo chính sách đợt tuyển. Sau tốt nghiệp đạt yêu cầu, người lao động được doanh nghiệp tiếp nhận ký hợp đồng và bố trí việc làm. ${master.income_commitment}. ${dossier.initial_application}. Khi có lịch nhập học, mang ${dossier.admission_documents.join(", ").toLocaleLowerCase("vi")}. ${dossier.missing_diploma}. Địa điểm nhập học: ${contact.admission_address}. Địa chỉ liên hệ, tư vấn: ${contact.address}.`;
}

function htmlDescription(role) {
  return `<p>${role.responsibility}</p><p>Công việc chính: ${responsibilityText(role)}.</p><p>Điều kiện: nam ${ageLabel} tuổi, cao từ ${heightLabel}, nặng từ ${criteria.weight_min_kg} kg; sức khỏe tốt, không cận thị và không mắc bệnh tim mạch, huyết áp hoặc bệnh về mắt ảnh hưởng đến công việc.</p><p>Không yêu cầu kinh nghiệm. Người đăng ký cần đọc và viết thành thạo; trình độ cụ thể được đối chiếu theo hệ đào tạo.</p><p>Thời gian học ${role.training_duration_current}. Người học thuộc chỉ tiêu được miễn kinh phí đào tạo, bố trí ba bữa/ngày với mức ăn 90.000 đồng/ngày, ký túc xá và hỗ trợ 7,5 triệu đồng/tháng theo chính sách đợt tuyển.</p><p>Sau tốt nghiệp đạt yêu cầu, doanh nghiệp tiếp nhận ký hợp đồng lao động và bố trí việc làm tại Quảng Ninh; Thu nhập bình quân 20–25 triệu đồng/tháng, tùy đơn vị, vị trí, ngày công và năng suất.</p><p>${dossier.initial_application}. Khi có lịch nhập học, mang ${dossier.admission_documents.join(", ").toLocaleLowerCase("vi")}. ${dossier.missing_diploma}. Địa điểm nhập học: ${contact.admission_address}. Địa chỉ liên hệ, tư vấn: ${contact.address}.</p>`;
}

function roleFaq(role) {
  return [
    [`${role.title} làm công việc gì?`, `${role.summary} Công việc cụ thể gồm ${responsibilityText(role)}.`],
    [`Học ${role.short} bao lâu?`, `Thời gian đào tạo hiện hành là ${role.training_duration_current}. Người chưa có kinh nghiệm được học từ nền tảng trước khi thực tập và nhận việc.`],
    [`Thu nhập ${role.short} là bao nhiêu?`, `${master.income_commitment}.`],
    [`Muốn đăng ký ${role.short} thì liên hệ ai?`, `Liên hệ Nguyễn Tử Linh – Trưởng phòng Tuyển sinh Miền Trung qua điện thoại hoặc Zalo 096 304 8585 để được kiểm tra điều kiện và hướng dẫn lịch nhập học.`],
  ];
}

function faqSection(role) {
  return `<section class="section job-section job-faq" id="cau-hoi-thuong-gap" aria-labelledby="job-faq-title"><div class="section-heading"><div><p class="eyebrow">CÂU HỎI NGƯỜI LAO ĐỘNG THƯỜNG TÌM</p><h2 id="job-faq-title">Hỏi nhanh về ${role.title.toLocaleLowerCase("vi")}</h2></div><p>Thông tin trả lời theo hồ sơ nghề và chính sách tuyển sinh đang áp dụng.</p></div><div class="condition-grid">${roleFaq(role).map(([question, answer], index) => `<article><span>${String(index + 1).padStart(2, "0")}</span><h3>${question}</h3><p>${answer}</p></article>`).join("")}</div></section>`;
}

function jobDetailsSection(role) {
  const taskItems = role.responsibilities.map((item, index) => `<li><span>${String(index + 1).padStart(2, "0")}</span><p>${item}</p></li>`).join("");
  const equipmentItems = role.equipment.map((item) => `<li>${item}</li>`).join("");
  return `<section class="section job-section" id="mo-ta-cong-viec" aria-labelledby="job-detail-title"><div class="section-heading"><div><p class="eyebrow">MÔ TẢ CÔNG VIỆC CHUẨN</p><h2 id="job-detail-title">${role.title} làm những công việc gì?</h2></div><p>${role.summary}</p></div><div class="dossier-board"><ol>${taskItems}</ol><aside><p class="eyebrow">THIẾT BỊ, DỤNG CỤ CHỦ YẾU</p><ul>${equipmentItems}</ul><p>${role.work_context}</p></aside></div><p class="job-source-note"><strong>Nguồn mô tả nghề:</strong> ${occupationSource.title}, số ${occupationSource.number}, ngày 22/02/2022 của ${occupationSource.issuer}. Thông tin tuyển sinh, thời gian học và thu nhập trên trang sử dụng chính sách hiện hành năm 2026.</p></section>`;
}
    ...
    "@graph": [
      {
        ...
        responsibilities: responsibilityText(role),
...
const jobs = roles.map((role) => ({
  ...
  training_duration: role.training_duration_current,
  ...
  occupation_source: { id: occupationSource.id, number: occupationSource.number, title: occupationSource.title, issuer: occupationSource.issuer, issued_date: occupationSource.issued_date },
}));

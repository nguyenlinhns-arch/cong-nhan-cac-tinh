...
if (count(home, 'href="/worker-info-finder.css?v=1"') !== 1) fail("Trang chủ: stylesheet tìm thông tin phải được nạp đúng một lần");
if (count(home, 'src="/worker-info-finder.js?v=1"') !== 1) fail("Trang chủ: script tìm thông tin phải được nạp đúng một lần");
...
for (const marker of ["focus-visible", "@media(max-width:900px)", "@media(max-width:720px)", "worker-check__result", "worker-find__province-row"]) {
...
try {
  ...
  if (checkResult.dataset.state !== "pass" || !checkResult.innerHTML.includes("Tiếp tục đăng ký") || !checkResult.innerHTML.includes("khám tuyển")) {
    fail("Tự kiểm tra: nhánh phù hợp sơ bộ không trả kết quả an toàn");
  }
  answers.set("health_screen", "review");
  checkForm.submit({ preventDefault() {} });
  if (checkResult.dataset.state !== "review" || !checkResult.innerHTML.includes("Nhắn Zalo để hỏi") || !checkResult.innerHTML.includes("chưa phải kết luận cuối cùng")) {

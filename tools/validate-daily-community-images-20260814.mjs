...
for (const article of dailyCommunityArticles20260814) {
  ...
  try {
    const sourceResponse = await fetch(source.sourceUrl, {headers, redirect: "follow", signal: AbortSignal.timeout(45_000)});
    const html = await sourceResponse.text();
    let extracted = extractSourceImage(html);
    let relationship = "OPEN_GRAPH";

    if (source.sourcePostId && source.sourceMediaId) {
      const postResponse = await fetch(`https://caodangtkv.edu.vn/wp-json/wp/v2/posts/${source.sourcePostId}?_fields=id,featured_media,link,title`, {headers, redirect: "follow", signal: AbortSignal.timeout(45_000)});
      const post = await postResponse.json();
      const mediaResponse = await fetch(`https://caodangtkv.edu.vn/wp-json/wp/v2/media/${source.sourceMediaId}?_fields=id,source_url,media_details`, {headers, redirect: "follow", signal: AbortSignal.timeout(45_000)});
    ...
    const imageResponse = await fetch(extracted || article.image, {headers: {...headers, range: "bytes=0-0", referer: encodeURI(source.sourceUrl)}, redirect: "follow", signal: AbortSignal.timeout(45_000)});
...
if (new Set(dailyCommunityArticles20260814.map((article) => article.image)).size !== dailyCommunityArticles20260814.length) errors.push("Ảnh đại diện trong lượt 14/08 bị trùng nhau");

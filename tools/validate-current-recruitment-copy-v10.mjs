...
for (const relative of corePages) {
  ...
  for (const [pattern, message] of forbidden) if (pattern.test(text)) add(relative, message);
  ...
  for (const match of html.matchAll(/20\s*[–-]\s*25\s*triệu/giu)) {
    ...
    const window = html.slice(Math.max(0, index - 220), Math.min(html.length, index + 320));

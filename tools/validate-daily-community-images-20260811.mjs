...
const errors = [];
const results = [];
for (const article of dailyCommunityArticles20260811) {
  ...
  try {
    ...
    const matched = response.status === 200 && extracted === article.image && imageResponse?.status === 200 && imageResponse.headers.get("content-type")?.startsWith("image/");
...
if (new Set(dailyCommunityArticles20260811.map((article) => article.image)).size !== dailyCommunityArticles20260811.length) errors.push("Ảnh đại diện trong lượt 11/08 bị trùng nhau");
